...
public class RrdGraph implements RrdGraphConstants {
	...
	private void collectInfo() {
		...
		for (int i = 0; i < gdef.comments.size(); i++) {
			if (gdef.comments.get(i) instanceof PrintText) {
				PrintText pt = (PrintText) gdef.comments.get(i);
				if (pt.isPrint()) {
	...
	private void drawRules() {
		...
		for (int i = 0; i < gdef.plotElements.size(); i++) {
			PlotElement pe = gdef.plotElements.get(i);
	...
	private void drawData() throws RrdException {
		...
		for (int i = 0; i < gdef.plotElements.size(); i++) {
			if (gdef.plotElements.get(i) instanceof SourcedPlotElement) {
				SourcedPlotElement source = (SourcedPlotElement) gdef.plotElements.get(i);
	...
	private void removeOutOfRangeRules() {
		for (int i = 0; i < gdef.plotElements.size(); i++) {
			PlotElement plotElement = gdef.plotElements.get(i);
	...
	private void fetchData() throws RrdException, IOException {
		...
		for (int i = 0; i < gdef.sources.size(); i++) {
			Source src = gdef.sources.get(i);
	...
	private boolean lazyCheck() {
		...
		if (gdef.lazy == false || !Util.fileExists(gdef.filename)) {
	...
	private void drawLegend() {
		if (!gdef.onlyGraph && !gdef.noLegend) {
			...
			for (int i = 0; i < gdef.comments.size(); i++) {
				CommentText c = gdef.comments.get(i);
