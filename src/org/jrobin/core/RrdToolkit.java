	...
	public static int forceZerosForNans(String sourcePath, String dsName) throws IOException, RrdException {
		...
		try {
			...
			for (int i = 0; i < datasources.length; i++) {
				String currentDsName = datasources[i].getDsName();
				if (!currentDsName.endsWith(DsDef.FORCE_ZEROS_FOR_NANS_SUFFIX)) {
					datasources[i].setDsName(currentDsName + DsDef.FORCE_ZEROS_FOR_NANS_SUFFIX);
					count++;
				}
			}
			return count;
	...
	private static void traverseDirectory(File directory, String extension, boolean recursive, List<String> list)
		...
		for (int i = 0; i < files.length; i++) {
			if (files[i].isDirectory() && recursive) {
				...
				traverseDirectory(files[i], extension, recursive, list);
			}
			else if (files[i].isFile() && files[i].getName().endsWith(extension)) {
