...
public class ProfileGraphsNew 
{
	...
	ProfileGraphsNew( PrintWriter out, JRobinTimeProfiler profiler )
	{
		...
		profiler.newGraphs = (fullStop - fullStart);
	...
	private void createDemo1() throws IOException, RrdException
	{
		...
		graph.saveAsPNG( "/time_demo1-" + DNUM + ".png" );		// Default size	
		
		stop = System.currentTimeMillis();
		profiler.profiles[demoNum - 1].totalCreation[DNUM] = (stop - start);
	
		addTimingInfo( 1, Util.timeList );
	}
	...
	private void createDemo3() throws IOException, RrdException
	{
		...
		def.setVerticalLabel("server load");
		...
		def.setGridRange( 0, 1, false );
		//def.setValueRange( 0, 1 );
		//def.setRigidGrid( false );
		def.setDefaultFontColor( Color.WHITE );
		def.setTitleFontColor( Color.WHITE );
		...
		// Create graph
		graph.setGraphDef(def);
		graph.saveAsPNG( "/time_demo" + demoNum + "-" + DNUM + ".png" );		// Default size	
