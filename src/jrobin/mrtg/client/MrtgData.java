...
class MrtgData {
	...
	void setHost(String mrtgHost) throws IOException, XmlRpcException {
	...
	LinkInfo getLinkInfo(int routerIndex, int linkIndex) {
		...
		lInfo.setSamplingInterval(getLinkSamplingInterval(routerIndex, linkIndex));
	...
	ServerInfo getServerInfo() {
		...
		info.setBadSavesCount(Integer.parseInt(serverInfo.get("badSavesCount").toString()));
	...
	int deleteRouter(RouterInfo routerInfo)
		throws XmlRpcException, IOException {
	...
	int addLink(RouterInfo routerInfo, LinkInfo linkInfo)
		throws XmlRpcException, IOException {
	...
	TreeModel getTreeModel() {
		...
		for(int routerIndex = 0; routerIndex < routerInfo.length; routerIndex++) {
			DefaultMutableTreeNode routerNode =
				new DefaultMutableTreeNode(routerInfo[routerIndex]);
			root.add(routerNode);
			...
			for(int linkIndex = 0; linkIndex < linkInfo.length; linkIndex++) {
				DefaultMutableTreeNode linkNode =
					new DefaultMutableTreeNode(linkInfo[linkIndex]);
